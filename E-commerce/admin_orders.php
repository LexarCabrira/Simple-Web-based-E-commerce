...
// --- 2. FETCH REAL STATS FOR WIDGETS ---
$rev_res = mysqli_query($conn, "SELECT SUM(total_price) as total FROM orders WHERE status='Delivered'");
$rev_row = mysqli_fetch_assoc($rev_res);
$net_revenue = number_format($rev_row['total'] ?? 0, 2);

$total_res = mysqli_query($conn, "SELECT COUNT(*) as total FROM orders");
...
// --- 3. FETCH ORDERS FOR THE TABLE (WITH FILTER) ---
...
<head>
    ...
    <title>Silverlocks Admin | Order Management</title>
    ...
    <style>
        ...
        .nav-link:hover, .nav-link.active { background: #ffffff; color: var(--sidebar-dark) !important; box-shadow: 0 4px 20px rgba(0,0,0,0.15); }
        
        .main-content { margin-left: 320px; padding: 40px 40px 40px 20px; }
        .glass-card { background: var(--glass-bg); backdrop-filter: blur(12px); border: 1px solid rgba(255, 255, 255, 0.5); border-radius: 24px; padding: 35px; box-shadow: 0 8px 32px 0 rgba(31, 38, 135, 0.07); }
        ...
        .filter-select:hover { background-color: #e9ecef; }
        ...
        .btn-manage { background: white; border: 1px solid #ddd; border-radius: 10px; padding: 6px 15px; font-size: 0.85rem; transition: 0.2s; text-decoration: none; color: black; }
        .btn-manage:hover { background: #000; color: #fff; }
    </style>
</head>
<body>
    ...
    <aside class="sidebar">
        ...
        <nav class="nav flex-column">
            ...
            <a class="nav-link active" href="admin_orders.php"><i class="fas fa-receipt"></i> Orders</a>
    ...
    <main class="main-content">
        <div class="d-flex justify-content-between align-items-end mb-5">
            <div>
                <h1 class="fw-bold mb-1" style="font-family: 'Playfair Display'; font-size: 2.5rem;">Order Management</h1>
                <p class="text-muted mb-0">Manage customer bakery orders and fulfillment.</p>
            </div>
        </div>

        <!-- STATS ROW -->
        <div class="row g-4 mb-5">
            <div class="col-xl-3 col-md-6"><div class="stat-widget shadow-sm"><div class="stat-icon-box icon-bg-sales"><i class="fas fa-wallet"></i></div><div><p class="text-muted small mb-0">Total Sales</p><h4 class="fw-bold mb-0">₱<?php echo $net_revenue; ?></h4></div></div></div>
            ...
            <div class="col-xl-3 col-md-6"><div class="stat-widget shadow-sm"><div class="stat-icon-box icon-bg-stock"><i class="fas fa-box-open"></i></div><div><p class="text-muted small mb-0">Alerts</p><h4 class="fw-bold mb-0">0 Items</h4></div></div></div>
        </div>

        <div class="glass-card">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h5 class="fw-bold mb-0" style="font-family: 'Playfair Display'; font-size: 1.4rem;">Recent Orders</h5>
                
                <!-- DROPDOWN FILTER -->
                <select class="filter-select shadow-sm" onchange="window.location.href='admin_orders.php?status=' + this.value;">
                ...
                <table class="table align-middle">
                    ...
                    <tbody>
                        ...
                        if(mysqli_num_rows($orders_result) > 0) {
                            while($item = mysqli_fetch_assoc($orders_result)) {
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="admin_view_order.php?id=<?php echo $item['id']; ?>" class="btn-manage">Manage</a>
                            </td>
                        </tr>
                        <?php 
                            }
                        } else {
                            echo "<tr><td colspan='5' class='text-center py-5 text-muted'>No orders found for '$status_filter'.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
